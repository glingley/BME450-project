...
class TextProcessor:
    ...
    def __init__(self, vocab_file):
        ...
        self.tokenize_regex = re.compile(r"(\\[a-zA-Z]+)|((\\)*[$-/:-?{-~!\"^_`\[\]])|(\w+)|(\S)")

    def split_into_tokens(self, formula: str):
        return [m.group(0) for m in re.finditer(self.tokenize_regex, formula)]
...
# Convolutional Encoder using ResNet-18
...
class Image2LatexModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        images, formulas = batch
        print(f"Test batch - Images shape: {images.shape}, Formulas shape: {formulas.shape}")
        formulas_target = formulas[:, 1:]
        formulas_input = formulas[:, :-1]

        output = self(images, formulas_input)
        output = output.view(-1, output.shape[-1])
        formulas_target = formulas_target.view(-1)

        loss = self.loss_fn(output, formulas_target)

        predicts = [self.text.split_into_tokens(self.decode(output[i], self.max_length)) for i in range(output.shape[0])]
        truths = [self.text.split_into_tokens(self.text.decode(formulas_target[i])) for i in range(formulas_target.shape[0])]
        ...
        return {"test_loss": loss, "test_edit_distance": edit_dist, "test_bleu4": bleu4, "test_exact_match": em}

    '''
    def decode(self, x, max_length):
        # Assuming x is a single image without batch dimension
        if len(x.shape) == 3:
            x = x.unsqueeze(0)
        ...
        for _ in range(max_length):
            ...
            next_token = output.argmax(-1)[:, -1].item()  # Get the last token from the sequence
        ...
        decoded_text = self.text.decode(predictions)
        return decoded_text
    '''

    def decode(self, x):
        """
        Greedy decoding for the transformer decoder.
        
        Args:
        x (Tensor): The input image tensor for the encoder.
        
        Returns:
        str: The decoded LaTeX string.
        """
        # Get the encoder outputs
        memory = self.encoder(x.unsqueeze(0))  # Assuming x is a single image without batch dimension
        ...
        # Convert the list of token indices to text using the provided text processing utility
        decoded_text = self.text.int2text(predictions)
        return decoded_text
...
class LatexDataModule(pl.LightningDataModule):
    def __init__(self, data_path, img_path, batch_size=32, num_workers=0, text_processor=None):
        super().__init__()
...
def main():
    ...
    trainer = pl.Trainer(max_epochs=10, callbacks=[checkpoint_callback], logger=[tensorboard_logger, csv_logger])
