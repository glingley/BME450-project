...
# LSTM Decoder for textual output from image features
class TextDecoder(pl.LightningModule):
    def __init__(self, encoder, hidden_dim, vocab_file):
        ...
        # Metrics initialization with specified tasks
        self.accuracy = Accuracy(task="multiclass", num_classes=len(self.tokenizer))
        self.precision = Precision(task='multiclass', num_classes=len(self.tokenizer))
        self.recall = Recall(task='multiclass', num_classes=len(self.tokenizer))
        self.f1_score = F1Score(task='multiclass', num_classes=len(self.tokenizer))
    ...
    def training_step(self, batch, batch_idx):
        ...
        outputs, targets = self(images, captions)
        loss = self.loss_fn(outputs.transpose(1, 2), targets)
        acc = self.accuracy(outputs.argmax(dim=2), targets)
        self.log('train_loss', loss)
        self.log('train_acc', acc)
        self.log('train_precision', self.precision(outputs.argmax(dim=2), targets))
        self.log('train_recall', self.recall(outputs.argmax(dim=2), targets))
        self.log('train_f1_score', self.f1_score(outputs.argmax(dim=2), targets))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_precision', self.precision(outputs.argmax(dim=2), targets))
        self.log('val_recall', self.recall(outputs.argmax(dim=2), targets))
        self.log('val_f1_score', self.f1_score(outputs.argmax(dim=2), targets))
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log('test_precision', self.precision(outputs.argmax(dim=2), targets))
        self.log('test_recall', self.recall(outputs.argmax(dim=2), targets))
        self.log('test_f1_score', self.f1_score(outputs.argmax(dim=2), targets))
...
class LatexDataModule(pl.LightningDataModule):
    def __init__(self, data_path, img_path, batch_size=16, num_workers=11, subsampleTrain=600, subsampleVal=60, subsampleTest=60):
        super().__init__()
...
# Train the model
# Define the train_model function as before
def train_model():
    ...
    data_module = LatexDataModule(data_path, img_path, batch_size, num_workers, subsampleTrain=600, subsampleVal=60, subsampleTest=60)
    model = TextDecoder(encoder, hidden_dim, vocab_file)

    # Setup TensorBoard Logger
    logger = TensorBoardLogger("Final_Project/tb_logs", name="image_to_text")

    # Setup dynamic checkpointing
    checkpoint_callback = ModelCheckpoint(
        monitor='val_loss',
        dirpath="Final_Project/model_checkpoints",
        filename='checkpoint-{epoch:02d}-{val_loss:.2f}-{batch_size}-{subsampleTrain}-{subsampleVal}-{subsampleTest}',
        save_top_k=1,
        mode='min',
        auto_insert_metric_name=False  # Ensure custom naming without automatic metric name insertion
    )
    ...
    hyperparams = {
        ...
        'subsample_train': data_module.subsampleTrain,
        'subsample_val': data_module.subsampleVal,
        'subsample_test': data_module.subsampleTest,
